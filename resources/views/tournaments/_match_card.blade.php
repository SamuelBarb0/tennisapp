...
<div x-data="{
    ...
    >
        ...
        <div class="pr {{ $isCancelled ? 'l' : ($isFinished ? ($p1Won ? 'w' : 'l') : ($isLive ? 'lv' : 'n')) }}"
            ...
            {{-- Fixed-width indicator slot RIGHT AFTER the name. Always reserves space so both player rows have identical structure and the score columns align vertically. --}}
            ...
            {{-- Fixed-width score block (always rendered; empty when no score). --}}
        ...
        <div class="pr {{ $isCancelled ? 'l' : ($isFinished ? ($p2Won ? 'w' : 'l') : ($isLive ? 'lv' : 'n')) }}"
            ...
            @if($isCancelled)
                <span class="text-[7px] font-bold text-gray-400 bg-gray-200 px-1.5 py-0.5 rounded shrink-0">CANC</span>
            @else
                <div class="flex items-center gap-0.5 shrink-0">
                    @foreach($setPairs as $pair)
