...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="flex flex-wrap gap-2 mb-8" x-data="{ active: '{{ request('type', '') }}' }">
        <a href="{{ route('tournaments.index') }}"
           class="px-5 py-2 rounded-full text-sm font-medium transition-all {{ !request('type') ? 'bg-[#0071E3] text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200' }}">
            Todos
        </a>
        ...
        <a href="{{ route('tournaments.index', ['type' => 'ATP']) }}"
           class="px-5 py-2 rounded-full text-sm font-medium transition-all {{ request('type') === 'ATP' ? 'bg-[#0071E3] text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200' }}">
            ATP Masters 1000
        </a>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <a href="{{ route('tournaments.show', $tournament) }}" class="group block bg-white rounded-3xl overflow-hidden shadow-sm hover-lift border border-gray-100 fade-in" style="animation-delay: {{ $loop->index * 0.05 }}s">
            <div class="h-44 bg-gradient-to-br {{ $tournament->type === 'GrandSlam' ? 'from-yellow-400 to-orange-500' : ($tournament->type === 'ATP' ? 'from-[#0071E3] to-blue-700' : 'from-purple-500 to-pink-500') }} flex items-center justify-center relative">
            ...
            <div class="p-5">
                <h3 class="font-semibold text-lg mb-2 group-hover:text-[#0071E3] transition-colors">{{ $tournament->name }}</h3>
                ...
                <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @endif
                        <span class="text-xs font-medium px-2 py-1 bg-blue-50 text-[#0071E3] rounded-lg">x{{ $tournament->points_multiplier }}</span>
                    </div>
                </div>
            </div>
        </a>
