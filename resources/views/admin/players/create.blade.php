    ...
    <form action="{{ route('admin.players.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="nationality_code" value="{{ old('nationality_code') }}" maxlength="3" placeholder="ESP, USA, ARG..." required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-tc-primary focus:border-transparent outline-none uppercase">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="ranking" value="{{ old('ranking') }}" min="1" required class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-tc-primary focus:border-transparent outline-none">
        ...
        <div>
            ...
            <textarea name="bio" rows="4" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-tc-primary focus:border-transparent outline-none resize-none">{{ old('bio') }}</textarea>
        ...
        <div class="flex items-center gap-3 pt-2">
            <button type="submit" class="px-6 py-2.5 bg-tc-primary text-white rounded-xl text-sm font-medium hover:bg-tc-primary-hover transition-colors">Crear jugador</button>
