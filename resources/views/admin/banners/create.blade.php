    ...
    <form action="{{ route('admin.banners.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
        ...
        <div>
            ...
            <select name="slot" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-tc-primary focus:border-transparent outline-none">
                @foreach(\App\Models\Banner::SLOTS as $slotKey => $slotCfg)
                    <option value="{{ $slotKey }}" {{ old('slot', $slot) === $slotKey ? 'selected' : '' }}>{{ $slotCfg['label'] }}</option>
                @endforeach
            </select>
        ...
        <div class="space-y-2">
            ...
            <label class="flex items-start gap-2 cursor-pointer">
                <input type="checkbox" name="show_stats" value="1" {{ old('show_stats', true) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-tc-primary focus:ring-tc-primary mt-0.5">
                <span class="text-sm">
