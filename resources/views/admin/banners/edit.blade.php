...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm space-y-5">
        @csrf
        @method('PUT')
        <div>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="media_type" class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-tc-primary focus:border-transparent outline-none">
                    <option value="image" {{ old('media_type', $banner->media_type) === 'image' ? 'selected' : '' }}>Imagen</option>
            ...
            @if($banner->image)
                <div class="mt-2 flex items-center gap-2">
                    @if($banner->media_type === 'video')
                        <video src="{{ asset('storage/' . $banner->image) }}" class="w-20 h-12 rounded-lg object-cover" muted></video>
                    @else
                        <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->title }}" class="w-20 h-12 rounded-lg object-cover">
                    @endif
                    <span class="text-xs text-gray-400">Archivo actual</span>
                </div>
            @endif
            @error('image') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">URL externa (opcional, ej. video alojado en CDN)</label>
            <input type="url" name="media_url" value="{{ old('media_url', $banner->media_url) }}" placeholder="https://..." class="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-tc-primary focus:border-transparent outline-none">
        ...
        <div class="space-y-2">
            ...
            <label class="flex items-start gap-2 cursor-pointer">
                <input type="checkbox" name="show_stats" value="1" {{ old('show_stats', $banner->show_stats ?? true) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-tc-primary focus:ring-tc-primary mt-0.5">
                <span class="text-sm">
                    <span class="font-medium">Mostrar estadísticas</span>
                    <span class="block text-[11px] text-gray-500">Solo aplica a heros (Home y Premios). En el hero del Home muestra el grid de Torneos/Jugadores/Puntos/Participantes; en Premios muestra el contador "puntos disponibles".</span>
                </span>
            </label>
        </div>
